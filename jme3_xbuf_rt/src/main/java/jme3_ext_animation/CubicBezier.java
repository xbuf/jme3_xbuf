...
public class CubicBezier {
	 ...
	 */
	static public float bezier1D(float p0, float h0, float p1, float h1, float t) {
		float rt = 1 - t;
		float rt2 = rt *rt;
		float t2 = t * t;
		return rt * rt2 * p0 + 3 * rt2 * t * h0 + 3 * rt * t2 * h1 + t * t2 *p1;
	}
	 ...
	 */
	static public float findYfromXBisec(float x, float yp0, float yp1, float xh0, float yh0, float xh1, float yh1, float xTolerance, int maxIteration ) {
		...
		float lower = 0f;
		float upper = 1f;
		float t = (upper + lower) / 2f;

		float xt = bezier1D(0, xh0, 1, xh1, t);


		for(int ite = maxIteration; Math.abs(x - xt) > xTolerance && ite > 0; ite--) {
			if(x > xt) {
				lower = t;
			} else {
				upper = t;
		...
		//return the y value.
		return bezier1D(yp0, yh0, yp1, yh1, t);
	}
}
